...
type TemplatesView = 'main' | 'preview' | 'stats' | 'edit';

export interface TemplatesTabProps {
  onSubViewChange?: (isInSubView: boolean) => void;
}

export function TemplatesTab({ onSubViewChange }: TemplatesTabProps = {}) {
  ...
  // Notify parent when entering/exiting sub-views
  useEffect(() => {
    if (onSubViewChange) {
      const isInSubView = currentView !== 'main';
      onSubViewChange(isInSubView);
    }
  ...
  // Render preview page
  if (currentView === 'preview' && selectedTemplateId) {
    return (
      <TemplatePreviewPage
  ...
  // Render stats page
  if (currentView === 'stats' && selectedTemplateId) {
    return (
      <TemplateStatsPage
        templateId={selectedTemplateId}
        onBack={handleBackToMain}
  ...
  // Render edit/create form as full-screen view
  if (currentView === 'edit') {
    return (
      <div className="space-y-4">
        {/* Navigation Header for Edit View */}
        <NavigationHeader
          title={isCreating ? 'Create New Template' : 'Edit Template'}
          onBack={cancelForm}
  ...
  // Render main template list view
  return (
    <div className="space-y-4">

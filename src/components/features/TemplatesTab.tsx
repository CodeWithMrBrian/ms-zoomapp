...
export function TemplatesTab() {
  ...
  const saveTemplate = () => {
    ...
    const templateData: Omit<Template, 'id' | 'user_id' | 'created_at' | 'updated_at'> = {
      ...
      tts_enabled: false, // Default: text-to-speech disabled (for GUI mockup)
      confidence_threshold: 0.8, // Default: 80% confidence threshold
      show_partial_results: true // Default: show partial results
    };
  ...
  const handleViewStats = (templateId: string) => {
    setSelectedTemplateId(templateId);
  ...
  const handleUseTemplate = (templateId: string) => {
    ...
    alert('This would start a new session with this template. Feature coming in next phase!');
  };
